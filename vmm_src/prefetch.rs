...
use crate::cuda_ffi::{self, CU_EVENT_DISABLE_TIMING};
use crate::slab::{RegionId, RegionState, SlabId};
...
impl PrefetchWorker {
    pub fn new(inner: Arc<AllocatorInner>, device_ordinal: i32) -> Self {
        ...
        let thread = std::thread::Builder::new()
            .name("stagehand-vmm-prefetch".into())
            .spawn(move || {
...
fn prefetch_loop(
    ...
    loop {
        ...
        match injector.steal() {
            crossbeam_deque::Steal::Success(req) => {
...
fn handle_prefetch(
    ...
    // Take cold mutex + read lock for prefetch
    ...
    // Eviction if needed — requires write lock
    if cold.mapped_bytes + needed > ceiling {
        ...
        let result = crate::eviction::evict_for_space(
        ...
        drop(slabs_w);
        // Re-acquire read lock and re-validate
        let slabs = match inner.slabs.read() {
            Ok(s) => s,
            Err(_) => return,
        };
...
fn do_prefetch_map(
    ...
    slabs: &[Option<crate::slab::Slab>],
    cold: &mut crate::allocator::ColdState,
    req: &PrefetchRequest,
    base_ptr: crate::cuda_ffi::CUdeviceptr,
    needed: usize,
    device_ordinal: i32,
) {
    // Re-validate after potential eviction
    let region_offset = {
        let slab = match slabs.get(req.slab).and_then(|s| s.as_ref()) {
            Some(s) => s,
            None => return,
