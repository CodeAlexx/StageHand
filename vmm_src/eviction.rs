...
use crate::slab::{RegionState, Slab};
...
pub fn evict_for_space(
    ...
    // Collect eviction candidates: (slab_idx, region_idx, priority, last_access)
    ...
    // Sort: lowest priority first, then oldest access first
    ...
    for (slab_idx, region_idx, _, _) in &candidates {
        ...
        if prev.is_err() {
            continue; // state changed (another eviction or mapping)
        }
        ...
        // Safe to evict: state is Unmapped AND refcount is 0.
        // No fast path can grab this region (state != Resident).
        ...
        let phys = match rm.phys_handle.take() {
            Some(h) => h,
            None => {
                // Defensive: already unmapped state set above
                continue;
            }
        };
        drop(rm);
        ...
        // state already set to Unmapped by CAS above
