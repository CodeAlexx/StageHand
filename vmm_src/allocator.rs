...
use crate::cuda_ffi::{self, CUcontext, CUdevice, CUmemAllocationProp, CUstream};
use crate::error::VmmError;
use crate::eviction;
use crate::handle::ResidentHandle;
use crate::prefetch::{PrefetchRequest, PrefetchWorker};
use crate::slab::{Region, RegionId, RegionState, Slab, SlabId};
...
impl SlabAllocator {
        ...
        // === FAST PATH: read lock + atomics only ===
        {
            ...
            if region.state.load(Ordering::Acquire) == RegionState::Resident as u8 {
                ...
                if region.state.load(Ordering::Acquire) == RegionState::Resident as u8 {
                    ...
                    // Update access clock (best-effort, no lock needed)
                    // TODO(skeptic): Under sustained contention (many threads hitting
                    // the fast path simultaneously), try_lock on cold and region.mutable
                    // will always fail.  This means last_access is NEVER updated for
                    // hot-path regions.  Eviction sorts by last_access ASC, so a region
                    // that is constantly accessed but never clock-stamped looks ancient
                    // and becomes the FIRST eviction victim — exactly backwards from
                    // correct LRU.  Possible fixes:
                    //   (a) Use an AtomicU64 access_clock with fetch_add — avoids cold
                    //       Mutex entirely for the clock increment.  Still need
                    //       region.mutable for the per-region stamp, but the clock
                    //       itself wouldn't be gated on the Mutex.
                    //   (b) Use a per-region AtomicU64 last_access (no Mutex needed for
                    //       reads in eviction either — just Relaxed loads).
                    //   (c) Accept the degradation and document it.  In practice the
                    //       training loop is single-threaded Python with GIL, so
                    //       contention is near-zero.  But this assumption may not hold
                    //       for multi-stream or multi-threaded consumers.
            ...
            // Not Resident or race lost — drop read lock, enter cold path
        }
        ...
        match state {
            ...
            RegionState::Unmapped => {
                ...
                if cold.mapped_bytes + needed > ceiling {
                    // Need write lock for eviction (mutates region mutable state)
                    drop(slabs);
                    ...
                    drop(slabs_w);
                    // Re-acquire read lock for mapping
                    let slabs = self.inner.slabs.read().map_err(|_| VmmError::InvalidSlab)?;
    ...
    pub fn destroy_slab(&self, slab_id: SlabId) -> Result<(), VmmError> {
        // Lock ordering: cold first, then slabs — same as ensure_resident_cold
        // and handle_prefetch. Prevents deadlock.
        let mut cold = self.inner.cold.lock().map_err(|_| VmmError::InvalidSlab)?;
        let mut slabs = self.inner.slabs.write().map_err(|_| VmmError::InvalidSlab)?;
    ...
    pub fn stats(&self) -> AllocatorStats {
        // Lock ordering: cold first, then slabs — consistent with ensure_resident_cold.
...
impl Drop for SlabAllocator {
    fn drop(&mut self) {
        // Shutdown prefetch worker eagerly — no more async mapping after this.
        if let Ok(mut pw) = self.inner.prefetch_worker.lock() {
            if let Some(mut worker) = pw.take() {
                worker.shutdown();
            }
        }
        // Actual CUDA resource cleanup happens in AllocatorInner::drop(),
        // which only runs when the last Arc goes away (i.e., all
        // ResidentHandles and DLPack tensors have been dropped).
    }
}

impl Drop for AllocatorInner {
    fn drop(&mut self) {
        // This runs only when the last Arc<AllocatorInner> is dropped,
        // meaning all ResidentHandles (and DLPack tensors) are gone.
        // All refcounts should be 0 at this point.

        // Shutdown prefetch worker if SlabAllocator::drop didn't run
        // (defensive — normally already shut down).
        ...
        // Best-effort cleanup of remaining slabs
